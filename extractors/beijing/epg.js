/**
 * 北京广播电视台节目单：取自央视网的节目单接口 api.cntv.cn/epg/getEpgInfoByChannelNew。
 *
 * 北京台自己没有：官网北京时间（btime.com，brtn.cn 也跳到这里）的电视页只有编辑挑的几条「节目预告」，
 * 页面脚本只调取流接口；电视频道还要登录才出现。央视网这个接口按代号 btv1…btv9 收着北京台各频道
 * （它自己的节目单页只列央视频道，这组代号是逐个试出来的），不用登录、不带签名；咪咕取央视频道
 * 节目单走的也是央视网。
 *
 ...
 * 不认识的代号回 { errcode: '1001', msg: 'params error' }。
 *
 * 和取流链路没有任何共享状态：频道与代号的对应写在本文件里，只用调用方注入的 fetch，
 * 不 import 项目里的其它模块——整份拿出去就能单独产出节目单。
 */

export const EPG_API = 'https://api.cntv.cn/epg/getEpgInfoByChannelNew'
// 一天一份实测 3–5 KB
const MAX_BYTES = 512 * 1024
const SHANGHAI_OFFSET_MS = 8 * 60 * 60 * 1000
const DAY_MS = 24 * 60 * 60 * 1000
const MINUTE_MS = 60 * 1000
const KEY_RE = /^btv\d{1,2}$/
const OFF_AIR = /^频道无节目$/
...
export function parseProgrammes(payload, key, day) {
...
export default {
  ...
  channels() {
    return EPG_CHANNELS.map(channel => ({ ref: `beijing-tv-${channel.slug}`, name: channel.name, key: channel.key }))
  },

  /** 取一个频道某一天（上海日期 YYYYMMDD）的节目，按开始时间升序。 */
  async programmes(key, day, { fetchImpl = fetch, timeoutMs = 10000 } = {}) {
    const date = String(day)
    if (!KEY_RE.test(String(key)) || !Number.isFinite(dayStartMs(date))) throw new Error('央视网节目单参数非法')
    ...
    try {
      ...
      return parseProgrammes(payload, String(key), date)
    } finally {
      clearTimeout(timer)
    }
  },
