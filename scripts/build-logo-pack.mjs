 ...
 *   node scripts/build-logo-pack.mjs --add <台标名> <图片文件或地址> --source <出处> [--kind official|platform|library]
...
function detectMime(buf) {
  ...
  if (/^\s*(<\?xml[^>]*>\s*)?(<!--[\s\S]*?-->\s*)*(<!DOCTYPE[^>]*>\s*)?<svg[\s>]/i.test(buf.subarray(0, 1024).toString('utf8'))) return 'image/svg+xml'
  return null
}
...
async function toPng(buf) {
  ...
  const out = await page.evaluate(async (src, max, vector) => {
    const img = new Image()
    img.src = src
    await img.decode()
    ...
    // 位图不放大；矢量图按长边铺满
    const scale = vector ? max / Math.max(w, h) : Math.min(1, max / Math.max(w, h))
  ...
  }, `data:${mime};base64,${buf.toString('base64')}`, MAX_SIDE, mime === 'image/svg+xml')
...
async function putLogo(index, name, buf, meta) {
  const { png, width, height } = await toPng(buf)
...
async function addManual(index, name, src, { source, kind, note }) {
  ...
  const result = await putLogo(index, name, buf, { origin: 'manual', source, kind: kind || 'official', ...(note ? { note } : {}) })
...
async function main() {
  ...
  try {
    if (args.add) {
      ...
      console.log(await addManual(index, args.add, args._[0], { source: args.source, kind: args.kind }))
      saveIndex(index)
    } else if (args.import) {
      const index = loadIndex()
      ...
      for (const [name, info] of Object.entries(sources)) {
        ...
        const file = files.find(f => f.slice(0, f.lastIndexOf('.')) === name && f !== 'sources.json')
        if (!file) { console.log(`缺文件：${name}`); continue }
        try {
          console.log(await addManual(index, name, path.join(args.import, file), {
            source: info.page || info.url, kind: info.kind, note: info.note,
          }))
        } catch (e) {
